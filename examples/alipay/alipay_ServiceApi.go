...
func FormatAliPayPublicKey() {
	...
	pKey := alipay.FormatAliPayPublicKey(aliPayPublicKey)
	fmt.Println(pKey)
}
...
func ParseAliPayNotifyResultAndVerifyAliPaySign(req *http.Request) {
	...
	//解析请求参数
	notifyReq, err := alipay.ParseAliPayNotifyResult(req)
	if err != nil {
		fmt.Println("err:", err)
	}
...
func GetCertSN() {
	...
	sn, err = alipay.GetCertSN("alipay_cert/alipayRootCert.crt")
	if err != nil {
		fmt.Println("err:", err)
		return
	}
